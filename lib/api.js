  ...
  async get(path = '/', query = {}, apikey, options = {}) {
    try {
      const res = await this.create.get(path, {
        params: (query || apikey) ? new URLSearchParams(Object.entries({
          ...query, ...(apikey ? {
            [apikey]: config.APIs[this.name].Key
          } : {})
        })) : '',
        ...options
      })
  ...
  async post(path = '', data = {}, apikey, options = {}) {
    try {
      if (!!data) {
        ...
        const res = await this.create.post(path + new URLSearchParams(Object.entries({
          ...(apikey ? {
            [apikey]: config.APIs[this.name].Key
          } : {})
        })), form, {
