   ...
   try {
      ...
      switch (command) {
         ...
         case 'help': {
            ...
            let text = `╭─「 ${conn.user.name} 」\n`
            text += `│ ${Func.greeting()} @${m.sender.split`@`[0]}👋\n`
            text += `│\n`
            text += `│ Tanggal: ${week} ${weton}, ${date}\n`
            text += `│ Tanggal Islam: ${dateIslamic}\n`
            text += `│ Waktu: ${time}\n`
            text += `│\n`
            ...
            return conn.sendMessageModify(m.from, text + '© Moon Bot', m, {
               largeThumb: true,
               url: db.setting.link
            })
         }
         ...
         case 'chatgpt': {
            try {
               ...
               m.react('🕒')
               var json = await Api.get('api/gpt-3.5-turbo', {
                  prompt: m.text
               })
