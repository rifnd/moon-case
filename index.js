    ...
    for (let tihh of fatihh) {
      if (tihh.isGroup == false) {
        if (tihh.status == "offer") {
          let pa7rick = await client.sendTextWithMentions(
            tihh.from,
            `*${client.user.name}* tidak bisa menerima panggilan ${
    ...
    try {
      for (let ciko of pea) {
        if (ciko.announce == true) {
          ...
          client.reply(
            ciko.id,
            `「 Group Settings Change 」\n\nGroup Subject telah diganti menjadi *${ciko.subject}*`,
            m
          );
        }
      }
